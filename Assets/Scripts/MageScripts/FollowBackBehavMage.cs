...
public class FollowBackBehavMage : StateMachineBehaviour
{
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        if (Vector3.Distance(playerPos.position, animator.transform.position) >= stoppingDistance) // теряет игрока и начинает патруль
        {
            //animator.transform.position = this.transform.position; // остается там где потерял игрока

            animator.SetBool("isPatroling", true); // возвращается в положение Patrol
        }
